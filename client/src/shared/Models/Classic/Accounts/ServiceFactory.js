...
const AndrewsDefaultService = require('./Andrews/AndrewsDefaultService')
  ...
  */
	static getClass(mailboxType, serviceType) {
		switch (mailboxType + ':' + serviceType) {
			// Trello
			case MAILBOX_TYPES.TRELLO + ':' + SERVICE_TYPES.DEFAULT:
				return TrelloDefaultService
			// Trello
			case MAILBOX_TYPES.ANDREWS + ':' + SERVICE_TYPES.DEFAULT:
				return AndrewsDefaultService

			// Slack
			case MAILBOX_TYPES.SLACK + ':' + SERVICE_TYPES.DEFAULT:
				return SlackDefaultService

			// Google
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.DEFAULT:
				return GoogleDefaultService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.CALENDAR:
				return GoogleCalendarService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.COMMUNICATION:
				return GoogleCommunicationService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.CONTACTS:
				return GoogleContactsService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.NOTES:
				return GoogleNotesService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.PHOTOS:
				return GooglePhotosService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.STORAGE:
				return GoogleStorageService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.DOCS:
				return GoogleDocsService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.SHEETS:
				return GoogleSheetsService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.SLIDES:
				return GoogleSlidesService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.ANALYTICS:
				return GoogleAnalyticsService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.VIDEO:
				return GoogleVideoService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.SOCIAL:
				return GoogleSocialService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.MESSENGER:
				return GoogleMessengerService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.CLASSROOM:
				return GoogleClassroomService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.MUSIC:
				return GoogleMusicService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.FI:
				return GoogleFiService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.ADMIN:
				return GoogleAdminService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.TEAM:
				return GoogleTeamService
			case MAILBOX_TYPES.GOOGLE + ':' + SERVICE_TYPES.PHONE:
				return GooglePhoneService

			// Microsoft
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.DEFAULT:
				return MicrosoftDefaultService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.CALENDAR:
				return MicrosoftCalendarService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.CONTACTS:
				return MicrosoftContactsService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.NOTES:
				return MicrosoftNotesService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.STORAGE:
				return MicrosoftStorageService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.DOCS:
				return MicrosoftDocsService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.NOTEBOOK:
				return MicrosoftNotebookService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.SHEETS:
				return MicrosoftSheetsService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.SLIDES:
				return MicrosoftSlidesService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.TEAM:
				return MicrosoftTeamService
			case MAILBOX_TYPES.MICROSOFT + ':' + SERVICE_TYPES.TASK:
				return MicrosoftTaskService

			// Generic
			case MAILBOX_TYPES.GENERIC + ':' + SERVICE_TYPES.DEFAULT:
				return GenericDefaultService

			// Container
			case MAILBOX_TYPES.CONTAINER + ':' + SERVICE_TYPES.DEFAULT:
				return ContainerDefaultService
