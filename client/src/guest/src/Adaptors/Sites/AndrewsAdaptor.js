...
class AndrewsAdaptor extends BaseAdaptor {
	/* **************************************************************************/
	// Class properties
	/* **************************************************************************/

	static get matches() {
		return [ 'http(s)\\://localhost(*)' ]
	}
	static get hasJS() {
		return true
	}
	...
	executeJS() {
		this.pdfObserver = new window.MutationObserver((mutations) => {
			const frames = document.body.querySelectorAll(
				'iframe.attachment-viewer-frame-preview-iframe[src*=".pdf"]:not(.wavebox-pdf-iframe-preview-enabled)'
			)
			...
			(evt) => {
				...
				if (setting) {
					if (setting === 'auto-open') {
						evt.stopPropagation()
						window.localStorage['wavebox_auto_open_pdf_preview'] = preview.querySelector(
							'.wavebox-pdf-auto-open-setting input[type="checkbox"]'
						).checked
					} else {
						evt.stopPropagation()
						evt.preventDefault()
