...
class WaveboxApp {
  ...
  */
	start() {
		...
		if (process.platform === 'darwin') {
			WaveboxDarwinDockBehaviour.setup()
		}
  ...
  */
	_syncFetchShouldOpenHidden() {
		if (settingsStore.getState().ui.openHidden) {
			return true
		}
		if (this[privArgv].hidden || this[privArgv].hide) {
			return true
		}
  ...
  */
	_handleHTTPBasicLogin = (evt, webContents, request, authInfo, callback) => {
		...
		const parentWindow = BrowserWindow.fromWebContents(
			webContents.hostWebContents ? webContents.hostWebContents : webContents
		)
  ...
  */
	restartApp = () => {
		this[privCloseBehaviour].restartApp()
	}
}
