...
import { CR_EXTENSION_PROTOCOL, CR_CONTENT_SCRIPT_XHR_ACCEPT_PREFIX } from 'shared/extensionApis'
import {
...
class CRExtensionRuntimeHandler extends EventEmitter {
  ...
  */
	startExtension(extension) {
		if (this.runtimes.has(extension.id)) {
			return
		}
  ...
  */
	_handleChromeExtensionBufferRequest = (request, responder) => {
		...
		if (!purl.hostname || !purl.pathname) {
			return responder()
		}
	...
	_handlePortConnect = (evt, extensionId, connectInfo) => {
		try {
			...
			evt.sender.once('render-view-deleted', () => {
				const backgroundContents = runtime.backgroundPage.webContents
				if (!backgroundContents || backgroundContents.isDestroyed()) {
					return
				}
  ...
  */
	getRuntimeData() {
		const data = Array.from(this.runtimes.keys()).reduce((acc, key) => {
  ...
  */
	getRuntimeContextMenuData() {
		return Array.from(this.runtimes.keys()).reduce((acc, key) => {
  ...
  */
	getAllContentScriptGuestConfigs() {
		return Array.from(this.runtimes.values()).map((runtime) => runtime.contentScript.guestConfig).filter((c) => !!c)
	}
  ...
  */
	contextMenuItemSelected(extensionId, contents, params) {
		const runtime = this.runtimes.get(extensionId)
		if (!runtime || !runtime.contextMenus) {
			return
		}
  ...
  */
	updateContentSecurityPolicy(requestUrl, responseHeaders) {
		...
		const matchingRuntimes = Array.from(this.runtimes.values()).filter((runtime) => {
			...
			if (!manifest.wavebox.hasContentSecurityPolicy) {
				return false
			}
  ...
  */
	_extractCSXHRAcceptHeader(requestHeaders) {
		...
		if (prevAccept === undefined) {
			return {}
		}
		if (prevAccept.indexOf(CR_CONTENT_SCRIPT_XHR_ACCEPT_PREFIX) === -1) {
			return { acceptHeader: prevAccept }
		}
  ...
  */
	updateCSXHRBeforeSendHeaders = (details) => {
		...
		if (!extensionId || !xhrToken) {
			return nextHeaders
		}

		// Check we have the credentials
		const runtime = this.runtimes.get(extensionId)
		if (!runtime || runtime.contentScript.xhrToken !== xhrToken) {
			return nextHeaders
		}
		...
		const matches = CRExtensionMatchPatterns.matchUrls(
			...
			purl.hostname,
			purl.pathname,
			[].concat.apply([], runtime.extension.manifest.contentScripts.map((cs) => cs.matches))
		)
		if (!matches) {
			return nextHeaders
		}
