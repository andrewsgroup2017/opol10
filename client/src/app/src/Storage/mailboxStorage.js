...
const CLASSIC_SERVICE_TYPE_TO_NEW = {
	...
	'ANDREWS:DEFAULT': SERVICE_TYPES.ANDREWS,
...
const CLASSIC_MAILBOX_KEY_TO_AUTH = {
	authAppKey: [ new Set([ 'TRELLO', 'ANDREWS' ]), 'authAppKey' ],
	authToken: [ new Set([ 'TRELLO', 'SLACK', 'ANDREWS' ]), 'authToken' ],
...
const CLASSIC_MAILBOX_KEY_TO_SERVICE = {
	...
	avatarCharacterDisplay: [ new Set([ 'TRELLO', 'ANDREWS' ]), 'serviceAvatarCharacterDisplay' ],
	initials: [ new Set([ 'TRELLO', 'ANDREWS' ]), 'initials' ],
	fullName: [ new Set([ 'TRELLO', 'ANDREWS' ]), 'fullName' ],
	username: [ new Set([ 'TRELLO', 'ANDREWS' ]), 'username' ],
	email: [ new Set([ 'TRELLO', 'ANDREWS', 'GOOGLE', 'MICROSOFT' ]), 'serviceDisplayName' ],
...
const CLASSIC_SERVICE_KEY_SKIP = {
	// keys we're just not checking
	type: true,
	restorableUrl: true,
	guestConfig: true,
...
class MailboxStorageBucket extends MigratingStorageBucket {
  ...
  */
	_loadRawMailboxes() {
		...
		return index.map((id) => rawData[id]).filter((mb) => !!mb).map((mb) => {
  ...
  */
	_allModelProps(model) {
		const systemKeys = new Set([ '__proto__' ])
		let propNames = []
  ...
  */
	_modelPropsCompare(a, b) {
		...
		if (
			typeof a === 'object' &&
			typeof a.cloneData === 'function' &&
			typeof b === 'object' &&
			typeof b.cloneData === 'function'
		) {
			if (JSON.stringify(a.cloneData()) === JSON.stringify(b.cloneData())) {
				return true
			}
		}
		return false
	...
	_convertMailboxGroup(mailbox, next) {
		...
		if (mailbox.serviceDisplayMode === 'TOOLBAR') {
			if (mailbox.serviceToolbarIconLayout === 'LEFT_ALIGN') {
				...
				// Covers the default of RIGHT_ALIGN
				nextMailbox.toolbarEndServices = Object.keys(nextServices)
			}
		} else {
			// Covers the default of SIDEBAR
  ...
  */
	_convertMailbox(mailbox) {
		const nextMailbox = {
			...
			showBadge:
				mailbox.showCumulativeSidebarUnreadBadge !== undefined
					? mailbox.showCumulativeSidebarUnreadBadge
					: mailbox.showUnreadBadge,
			badgeColor:
				mailbox.cumulativeSidebarUnreadBadgeColor !== undefined
		...
		if (mailbox.type === 'CONTAINER') {
			...
			nextMailbox.openGoogleDriveLinksWithExternalBrowser =
				(mailbox.windowOpenUserConfig || {}).googledrive === true
			...
			nextMailbox.openGoogleDriveLinksWithExternalBrowser =
				mailbox.openDriveLinksWithExternalBrowser !== undefined
		...
		} else if (mailbox.type === 'ANDREWS') {
			nextMailbox.displayName = 'Andrews'
		}
  ...
  */
	_convertService(mailbox, service) {
		...
		const nextService = {
			id: uuid.v4(),
			type:
				typeof CLASSIC_SERVICE_TYPE_TO_NEW[fqClassicType] === 'function'
			...
			sleepable:
				service.sleepable === undefined && mailbox.type !== 'CONTAINER' && service.type === 'DEFAULT'
					? false
					: service.sleepable,
		...
		if (service.type === 'DEFAULT') {
			...
			if (mailbox.type === 'CONTAINER') {
				...
				nextService.displayName =
					(mailbox.usePageTitleAsDisplayName ? mailbox.pageTitle : undefined) || mailbox.displayName
				nextService.hasNavigationToolbar = service.hasNavigationToolbar
	...
	_validateMigration(prevMailboxesJS, nextJS) {
		...
		try {
			...
			if (prevMailboxes.length !== nextMailboxes.length) {
				report.push({
					level: 'fatal',
					message: `Mailbox counts are not equal ${prevMailboxes.length} !== ${nextMailboxes.length}`
				})
				return report
			}
			...
			mailboxes.forEach(([ prev, next ]) => {
				if (prev.services.length !== next.services.length) {
					report.push({
						level: 'fatal',
						ctx: prev.mailbox.id,
						message: `Service counts are not equal ${prev.services.length} !== ${next.services.length}`
					})
					serviceCountCheckFail = true
				}
			})
			if (serviceCountCheckFail) {
				return report
			}
			...
			mailboxes.forEach(([ prev, next ]) => {
				this._allModelProps(prev.mailbox).forEach((prevK) => {
					...
					if (CLASSIC_MAILBOX_KEY_DEPRICATED.has(prevK)) {
						return
					}
					if (CLASSIC_MAILBOX_KEY_SKIP.has(prevK)) {
						return
					}
				...
				prev.services.map((prevService, index) => {
					...
					this._allModelProps(prevService).forEach((prevK) => {
						...
						if (CLASSIC_SERVICE_KEY_DEPRICATED.has(prevK)) {
							return
						}
						if (CLASSIC_SERVICE_KEY_SKIP[prevK] === true) {
							return
						}
						if (CLASSIC_SERVICE_KEY_SKIP[prevK] === `${prev.mailbox.type}:${prevService.type}`) {
							return
						}
  ...
  */
	_generateMigrationReport(prevMailboxes, next) {
		try {
			...
			const { pass, fail } = migrationReport.reduce(
				(acc, l) => {
					if (l.level === 'pass') {
						acc.pass++
					} else {
			...
			const str = [
				...
				.concat(
					migrationReport.map((l) => {
						let extra = MIGRATION_PRINT_EXTRA ? (l.extra ? '\n  >' + l.extra.join('\n  >') : '') : ''
  ...
  */
	startMigration() {
		...
		prevMailboxes.forEach((mailbox) => {
			this._convertMailboxGroup(mailbox, next)
		})
