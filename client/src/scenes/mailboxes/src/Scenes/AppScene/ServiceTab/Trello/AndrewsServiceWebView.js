...
import { andrewsActions } from 'stores/andrews'

const REF = 'mailbox_tab'

export default class AndrewsServiceWebView extends React.Component {
	/* **************************************************************************/
	// Class
	/* **************************************************************************/
  ...
  */
	handleOpenItem = (evt) => {
		if (evt.serviceId === this.props.serviceId) {
			if (evt.data.boardId && evt.data.cardId) {
				this.refs[REF].loadURL(`https://localhost/card/board/a/${evt.data.boardId}/${evt.data.cardId}`)
			} else if (evt.data.board) {
				this.refs[REF].loadURL(`https://localhost/board/a/${evt.data.boardId}`)
			} else {
				this.refs[REF].loadURL('https://localhost')
			}
			...
			andrewsActions.syncServiceNotificationsAfter.defer(this.props.serviceId, 1000 * 5)
		}
	}
	...
	render() {
		...
		return <CoreServiceWebViewHibernator ref={REF} mailboxId={mailboxId} serviceId={serviceId} />
	}
}
