...
class ServiceWizardScene extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (
			this.props.match.params.serviceType !== nextProps.match.params.serviceType ||
			this.props.match.params.accessMode !== nextProps.match.params.accessMode
		) {
			this.setState({
	...
	render() {
		...
		return (
			<WizardStepperDialog open={open} steps={steps} currentStep={currentStep}>
				...
				{currentStep === 1 ? <WizardAuth /> : undefined}
				{currentStep === 2 ? (
					<WizardConfigure serviceId={match.params.serviceId} onRequestCancel={this.handleClose} />
				) : (
					undefined
				)}
			</WizardStepperDialog>
