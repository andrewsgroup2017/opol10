...
import AndrewsServiceSettings from './Andrews/AndrewsServiceSettings'
...
class ServiceSettingsSection extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (this.props.serviceId !== nextProps.serviceId) {
			this.setState(this.extractStateForMailbox(nextProps.serviceId, accountStore.getState()))
		}
	}
	...
	userChanged = (userState) => {
		this.setState(this.extractStateForService(this.props.serviceId, accountStore.getState()))
	}

	accountChanged = (accountState) => {
		this.setState(this.extractStateForService(this.props.serviceId, accountState))
	}
  ...
  */
	renderSettings(serviceId, serviceType, showRestart, onRequestEditCustomCode) {
		...
		switch (serviceType) {
			...
			case SERVICE_TYPES.ANDREWS:
				RenderClass = AndrewsServiceSettings
				break
	...
	render() {
		const { classes, serviceId, showRestart, onRequestEditCustomCode, ...passProps } = this.props
		const { hasService, mailboxId, logo, displayName, humanizedType, isRestricted, serviceType } = this.state
		if (!hasService) {
			return false
		}
					...
					<div>
						<Tooltip title="Remove">
							<div>
								<IconButton
									onClick={() => {
										window.location.hash = `/mailbox_service_delete/${mailboxId}/${serviceId}`
									}}
								>
									<DeleteIcon />
								</IconButton>
