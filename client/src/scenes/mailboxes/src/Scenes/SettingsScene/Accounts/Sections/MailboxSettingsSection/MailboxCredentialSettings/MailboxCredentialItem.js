...
import FABAndrewsIcon from 'wbfa/FABAndrews'
...
export default class MailboxCredentialItem extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (this.props.authId !== nextProps.authId) {
			this.setState(this.extractStateForMailbox(nextProps.authId, accountStore.getState()))
		}
	}
	...
	accountChanged = (accountState) => {
		this.setState(this.extractStateForMailbox(this.props.authId, accountState))
	}
  ...
  */
	renderListItemText(classes, auth) {
		...
		} else if (auth.namespace === AndrewsAuth.namespace) {
			TypeIcon = FABAndrewsIcon
		}
  ...
  */
		...
		if (auth.namespace === MicrosoftAuth.namespace) {
			return (
				<Select
					...
					renderValue={(value) =>
						value === 'true' ? 'Personal Account (Outlook)' : 'Corporate Account (Office365)'}
					onChange={(evt) => {
	...
	render() {
		const { authId, divider, showRestart, classes, className, ...passProps } = this.props
		...
		return (
			<SettingsListItem divider={divider} className={classNames(className, classes.root)} {...passProps}>
				<div className={classes.rootLine1}>
					{this.renderListItemText(classes, auth)}
					{this.renderDeleteButton(classes, authId)}
				</div>
				<div className={classes.rootLine2}>{this.renderSpecificOptions(classes, authId, auth)}</div>
			</SettingsListItem>
		)
	}
}
