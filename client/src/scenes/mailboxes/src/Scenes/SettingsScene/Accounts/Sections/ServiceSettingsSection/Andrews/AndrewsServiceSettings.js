...
import TrelloServiceReducer from 'shared/AltStores/Account/ServiceReducers/AndrewsServiceReducer'
import AccountCircleIcon from '@material-ui/icons/AccountCircle'
import ServiceAdvancedSection from '../Common/ServiceAdvancedSection'

export default class AndrewsServiceSettings extends React.Component {
	/* **************************************************************************/
	// Class
	/* **************************************************************************/
  ...
  */
	...
	render() {
		...
		const { hasService, homeBoardId, boards } = this.state
		if (!hasService) {
			return false
		}
			...
			<div {...passProps}>
				<SettingsListSection title="Account" icon={<AccountCircleIcon />}>
					<SettingsListItemSelect
						...
						options={[ { value: 'default', label: 'Andrews Home (Default)' } ].concat(
							boards.map((board) => {
