...
import {
	...
	WB_AUTH_ANDREWS_COMPLETE,
	WB_AUTH_TRELLO_ERROR,
	WB_AUTH_ANDREWS_ERROR,
...
class AccountActions extends RendererAccountActions {
	...
	startAttachNewService(attachTarget, serviceType, accessMode) {
		...
		if (serviceType === 'GOOGLE') {
			if (accessMode === 'GMAIL') {
				...
				return {
					attachTarget: attachTarget,
					serviceType: SERVICE_TYPES.GOOGLE_INBOX,
					accessMode: accessMode || '_'
				}
			}
			...
			return {
				attachTarget: attachTarget,
				serviceType: SERVICE_TYPES.MICROSOFT_MAIL,
				accessMode: accessMode || '_'
			}
		}
	}
  ...
  * @param payload: the data that came across the ipc
  */
	authAndrewsSuccess(evt, payload) {
		return payload
	}
...
ipcRenderer.on(WB_AUTH_ANDREWS_COMPLETE, actions.authTrelloSuccess)
ipcRenderer.on(WB_AUTH_ANDREWS_ERROR, actions.authFailure)
