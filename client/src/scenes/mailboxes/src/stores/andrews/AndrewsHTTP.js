...
class AndrewsHTTP {
  ...
  */
	static fetchAccountProfile(appKey, auth) {
		...
		return Promise.resolve()
			.then(() => window.fetch('https://localhost/1/members/me?' + query))
			.then((res) => (res.ok ? Promise.resolve(res) : Promise.reject(res)))
			.then((res) => res.json())
	}
  ...
  */
	static fetchUnreadNotifications(appKey, auth) {
		...
		return Promise.resolve()
			.then(() => window.fetch('https://localhost/1/members/me/notifications/?' + query))
			.then((res) => (res.ok ? Promise.resolve(res) : Promise.reject(res)))
			.then((res) => res.json())
	}
  ...
  */
	static fetchBoards(appKey, auth, fields = 'all', filter = 'all') {
		...
		return Promise.resolve()
			.then(() => window.fetch('https://localhost/1/members/me/boards/?' + query))
			.then((res) => (res.ok ? Promise.resolve(res) : Promise.reject(res)))
			.then((res) => res.json())
	}
}

export default AndrewsHTTP
