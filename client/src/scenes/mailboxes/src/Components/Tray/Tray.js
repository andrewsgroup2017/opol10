...
export default class Tray extends React.Component {
	...
	static propTypes = {
		// Careful we're strict in shouldComponentUpdate
	...
	componentWillMount() {
		...
		if (IS_GTK_PLATFORM && launchTraySettings.gtkUpdateMode === GTK_UPDATE_MODES.STATIC) {
			...
			const resizedImage = image.resize({
				width: launchTraySettings.iconSize,
				height: launchTraySettings.iconSize
			})
  ...
  */
	handleClick = (evt, bounds) => {
		...
		if (SUPPORTS_ADDITIONAL_CLICK_EVENTS) {
			...
			const action = evt.altKey || evt.ctrlKey || evt.shiftKey || evt.metaKey ? altClickAction : clickAction
  ...
  */
	shouldComponentUpdate(nextProps, nextState) {
		const propsDiff = [ 'unreadCount', 'hasUnreadActivity' ].findIndex((k) => this.props[k] !== nextProps[k]) !== -1
		if (propsDiff) {
			return true
		}
		...
		if (this.state.ctxMenuSig !== nextState.ctxMenuSig) {
			return true
		}
  ...
  */
	renderContextMenu() {
		...
		const template = [].concat(
			[
				...
				{ label: 'Show Mini', click: this.handleShowPopoutWindowMode },
				{ type: 'separator' },
	...
	render() {
		const { unreadCount, hasUnreadActivity, traySettings, launchTraySettings } = this.props
