$(function () {
  ...
  $rootScope.sendMail = function () {
    ...
    if (!email) {
      ...
      Backendless.Messaging.sendEmail($rootScope.table, bodyParts, [email], false)
        .then(createPopup('Email sent', 'successPopup'))
        .catch(e => createPopup(e.message, 'error'));
    }
  };
  ...
  .then(successCallshowHide)
      .catch(e => createPopup(e.message, 'error'));
  }

  $rootScope.runCode = function () {
    if ($rootScope.type == 'Update') {
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).findFirst()
        .then(data => $rootScope.tablesList[$rootScope.table](data).save())
    .then(successCallshowHide)
        .catch(e => {
  ...
  $rootScope.runSimpleFindCode = function () {
    if ($rootScope.findType == 'find') {
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).find()
        .then(response => {
        $rootScope.showHide('#attrContainer', '#simpleFind');
      $rootScope.data = response;
      $rootScope.props = response[0];
      $rootScope.parsedData = {};
    ...
    .catch(e => createPopup(e.message, 'error'));
      ...
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).findFirst()
        .then(response => resultCallback(response))
    .catch(e => createPopup(e.message, 'error'));
    } else if ($rootScope.findType == 'findLast') {
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).findLast()
        .then(response => resultCallback(response))
    .catch(e => createPopup(e.message, 'error'));
    }
  };
  ...
  $rootScope.showCheckTable = function () {
    ...
    if ($rootScope.complexFindType == 'findSort') {
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).findFirst()
        .then(response => {
        $rootScope.propsToCheck = response;
      $rootScope.checked = {};
      $rootScope.parsedData = {};
  ...
  $rootScope.runComplexFindCode = function () {
    ...
    if ($rootScope.complexFindType == 'findSort') {
      dataQuery.setProperties($rootScope.checked);
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).find(dataQuery)
        .then(response => {
        $rootScope.sortData = response;
    ...
    .catch(e => createPopup(e.message, 'error'));
      ...
      dataQuery.setRelated($rootScope.checked);
      Backendless.Persistence.of($rootScope.Classes[$rootScope.table]).find(dataQuery)
        .then(objectsWithLoadedRelations => {
        $rootScope.relatedDataToShow = {};
      $rootScope.relatedData = objectsWithLoadedRelations
      objectsWithLoadedRelations.forEach(function (obj) {
          ...
          $.each($rootScope.checked, function (key, checkedRel) {
            ...
            if (checkedRel == propKey) {
              if (isArray(obj[propKey])) {
                if (obj[propKey].length > 0) {
                  $rootScope.relatedDataToShow[checkedRel] = obj[propKey][0];
                }
              } else if (obj) {
                $rootScope.relatedDataToShow[checkedRel] = obj[propKey];
              }
            }
          });
        }
    ...
    .then(() => {
      ...
      for (var obj in $rootScope.relatedDataToShow) {
        ...
        for (var key in $rootScope.relatedDataToShow[obj]) {
          if (value && value.toString().search(/function \(/) == -1 && value.toString().search(
              /function\(/) == -1 && key != '___class' && key != '__meta') {
    ...
    if ($rootScope.complexFindType == 'findSort') {
      ...
      for (var i = 0, len = $rootScope.sortData.length; i < len; i++) {
        for (var k in $rootScope.sortData[i]) {
          if (k == key) {
            if (isArray($rootScope.sortData[i][key])) {
              $rootScope.sortData[i][key] = parsingArrays(html, $rootScope.sortData[i][key]);
            } else if (isObject($rootScope.sortData[i][key])) {
