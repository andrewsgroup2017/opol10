;(function($, H, Backendless) {
	...
	var App = {
		...
		search: function(e) {
			...
			Backendless.Persistence
				.of(TABLE_NAME)
				.find(Backendless.DataQueryBuilder.create().setWhereClause("name like '%" + searchQuery + "%'"))
		...
		addSubscriptions: function() {
			...
			if (list.length) {
				...
				Backendless.Persistence
					.of(TABLE_NAME)
					.addRelation(OBJECT_ID, COLUMN + ':' + TABLE_NAME, list)
		...
		removeSubscription: function(e) {
			...
			if (list.length) {
				...
				Backendless.Persistence
					.of(TABLE_NAME)
					.deleteRelation(OBJECT_ID, COLUMN, list)
		...
		refreshSubscriptionsList: function() {
			...
			Backendless.Persistence
				...
				.then(this.renderSubscriptions, this.onFail)
		},
	...
	App.init(APPLICATION_ID, SECRET_KEY)
