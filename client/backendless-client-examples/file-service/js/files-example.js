;(function(Backendless, $) {
	var APPLICATION_ID = '37480AA4-6BC0-AF1F-FF03-986DFDE80200'
	var SECRET_KEY = '329C3E5A-CAB0-7E37-FFFC-774374E7F500'
	...
	if (!APPLICATION_ID || !SECRET_KEY)
		alert(
			'Missing application ID or secret key arguments. Login to Backendless Console, select your app and get the ID and key from the Manage > App Settings screen. Copy/paste the values into the Backendless.initApp call located in FilesExample.js'
		)
	...
	function renderItems(items) {
		...
		$.each(items, function(index, value) {
			...
			var li =
				...
				value.url +
				'" >' +
				decodeURIComponent(protectXSS(name)) +
				'</a>' +
				'</div>' +
				'</div>' +
				'</li>'
	...
	function uploadFile() {
		var requests = files.map(function(file) {
			return Backendless.Files.upload(file, TEST_FOLDER, true).then(function(result) {
	...
	function deleteSelectedFiles() {
		...
		$selectedElements.each(function(index, element) {
			removeRequests.push(
				Backendless.Files.remove(element.src).then(function() {
