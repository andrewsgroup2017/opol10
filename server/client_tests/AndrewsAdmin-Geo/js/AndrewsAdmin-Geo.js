...
if (!APPLICATION_ID || !API_KEY)
    alert("Missing application ID or api key arguments. Login to Backendless Console, select your app and get the ID and key from the Manage > App Settings screen. Copy/paste the values into the Backendless.initApp call located in AndrewsAdmin-Geo.js");
    
    var loadedCats = "";

    Backendless.Geo.getCategories().then(function(cats) {
        if (cats.length > 0) {
            for(var obj in cats) {
                if (cats[obj].size > 0) {
            ...
            $('.index .tables').html("<div class='noPointsBlock'>No Geo categories in your Application. Go to Backendless Console to Add Category</div>");
        };
    }).then(function() { showInfoForEach('.index'); })
                ...
                $removed.each(function(){
                    ...
                    for(var key in pointsArr.arr){
                        if(key == $(this).attr('class').split(" ")[1].replace(/p/,"")){
                            try{
                                Backendless.Geo.deletePointSync(pointsArr.ids[key]);
                            } catch(e){
