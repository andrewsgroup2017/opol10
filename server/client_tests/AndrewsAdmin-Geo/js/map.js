...
function GMapLoad(target) {
  if ($('#' + target + 'Map').html().length > 0) {
    ...
    var cats = Backendless.Geo.getCategoriesSync();
    for (var obj in cats) {
      if (target.toLowerCase().replace("category", "") == cats[obj].name.toLowerCase()) {
        ...
        var result = Backendless.Geo.findSync(query);
        for (var point in result) {
          arr[point] = new google.maps.LatLng(result[point].latitude.toFixed(
            5), result[point].longitude.toFixed(5));
          ids[point] = result[point].objectId;
          metadata[point] = result[point].metadata;
          centerLong += result[point].longitude;
          centerLat += result[point].latitude;
        }
        pointsSize = result.length;
      }
    }
...
function LoadPoints(target, map) {
  ...
  for (var obj in cats) {
    if (target.toLowerCase().search(cats[obj].name.toLowerCase()) > 0) {
      var result = Backendless.Geo.findSync({
        categories     : cats[obj].name,
        includeMetadata: true
      });
      for (var point in result) {
...
function GMapBoundaries(map, condition) {
  ...
  for (var counter = 0; counter < markers.length; counter++) {
    ((markers[counter].position.lng() > ne.lng()) || (markers[counter].position.lng() < sw.lng())) ? markers[counter].setMap(
      null) : markers[counter].setMap(map);
  }
  var geoQuery =
      {
    ...
    if (result.length == 0) {
      ...
      for (var i = 0; i < result.length; i++) {
        for (var p = 0; p < markers.length; p++) {
          ...
          if ((lat == result[i].latitude) && (lng.toFixed(6) == result[i].longitude)) {
...
function GMapTryCatch(geoQuery) {
  try {
    var result = Backendless.Geo.findSync(geoQuery);
    return result;
  } catch (e) {
...
function GMapRadius(map, units, radius, target, condition) {
  ...
  google.maps.event.addListener(cityCircle, 'radius_changed', function() {
    ...
    var newRadius = (units == "MILES") ? parseInt(cityCircle.radius, 10) / 1600 : parseInt(cityCircle.radius,
      10) / 1000;
    ...
    if (result.length == 0) {
      ...
      for (var i = 0; i < result.length; i++) {
        for (var p = 0; p < markers.length; p++) {
          if ((markers[p].position.lat() == result[i].latitude) && (markers[p].position.lng().toFixed(
              6) == result[i].longitude)) {
            markers[p].setMap(map);
          }
        }
      }
  ...
  google.maps.event.addListener(cityCircle, 'center_changed', function() {
    var newRadius = (units == "MILES") ? parseInt(cityCircle.radius, 10) / 1600 : parseInt(cityCircle.radius,
      10) / 1000;
    for (var counter = 0; counter < markers.length; counter++) {
      ((markers[counter].position.lat() > ne.lat()) || (markers[counter].position.lat() < sw.lat())) ? markers[counter].setMap(
        null) : markers[counter].setMap(map);
    }
    $("#editor4").next().find('.latitude').text(cityCircle.center.lat());
