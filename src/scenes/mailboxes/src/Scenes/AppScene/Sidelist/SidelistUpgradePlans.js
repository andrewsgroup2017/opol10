  ...
  */
	generatePopupContent(classes, currentPlan, expiryDays) {
		...
		if (currentPlan === User.PLANS.FREE) {
			...
			if (expiryDays < 0) {
				text = [ 'Your trial has expired.', 'Enjoy the best of Wavebox by upgrading now!' ]
			} else if (expiryDays === 0) {
				text = [ 'Your trial is about to expire.', 'Upgrade now to continue enjoying the best of Wavebox' ]
			} else {
				text = [ `Your trial will expire in ${expiryDays} days.`, 'Upgrade to enjoy the best of Wavebox' ]
			}
		} else if (currentPlan === User.PLANS.PRO) {
			if (expiryDays < 0) {
				text = [ 'Your Pro membership has expired.', 'Enjoy the best of Wavebox by upgrading now!' ]
			} else if (expiryDays === 0) {
				text = [
		...
		return (
			<div className={classes.popoverContentContainer} onClick={this.handleUpgrade}>
				<div>{text.map((t, i) => <p key={i}>{t}</p>)}</div>
				<div className={classes.popoverMoreButton}>Find out more</div>
			</div>
		)
	}
	...
	render() {
		...
		const { expiresInDays, currentPlan, generatedId, buttonHover, tooltipHover, sidebarSize } = this.state
					...
					<div className={classes.compositeIconContainer}>
						...
						<div
							className={classNames(
								classes.remainingText,
								formattedDays.length === 2 ? classes.remainingText2Char : classes.remainingText3Char
							)}
						>
							{formattedDays}
						</div>
