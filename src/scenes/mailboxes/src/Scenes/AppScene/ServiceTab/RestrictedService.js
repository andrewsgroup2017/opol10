...
class RestrictedService extends React.Component {
	...
	render() {
		const { classes, mailboxId, serviceId, className, ...passProps } = this.props
		const { displayName } = this.state

		return (
			<div className={classNames(classes.root, className)} {...passProps}>
				<FASGemIcon className={classes.proIcon} />
				<h1 className={classes.title}>Andrews Admin</h1>
				<p className={classes.text}>{`You are logged out`}</p>
				<Button
					variant="raised"
					color="primary"
