...
import { AuthGoogle, AuthMicrosoft, AuthSlack, AuthTrello, AuthWavebox, AuthAndrews } from 'AuthProviders'
...
import { UISettings, TraySettings } from 'shared/Models/Settings'
...
class MailboxesWindow extends WaveboxWindow {
  ...
  */
	...
	constructor() {
		...
		this.authAndrews = new AuthAndrews()
  ...
  */
	...
	get rootWebContentsHasContextMenu() {
		return false
	}
  ...
  */
	create(hidden = false) {
		...
		super.create(this.generateWindowUrl(), {
			...
			fullscreenable: true,
			titleBarStyle:
				process.platform === 'darwin' && settingsState.launched.ui.showTitlebar === false
					? 'hidden'
					: 'default',
			frame: settingsState.launched.ui.showTitlebar,
			title: 'Andrews',
			...(process.platform === 'darwin' &&
			settingsState.launched.ui.vibrancyMode !== UISettings.VIBRANCY_MODES.NONE
				? {
