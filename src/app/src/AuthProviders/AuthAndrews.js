  ...
  */
	promptUserToGetAuthorizationCode(credentials, partitionId) {
		return new Promise((resolve, reject) => {
			...
			waveboxOauthWin.create('https://andrews.com/login', {
				useContentSize: true,
				center: true,
				show: true,
				resizable: false,
			...
			oauthWin.webContents.on('did-navigate', (evt, nextURL) => {
				...
				if (hostname === 'andrews.com' && pathname === '/') {
					oauthWin.hide()
					oauthWin.loadURL('https://andrews.com/app-key')
				}
			})
			...
			oauthWin.webContents.on('will-navigate', (evt, nextUrl) => {
				const { hostname, pathname } = new URL(nextUrl)
				if (hostname === 'andrews.com' && pathname.startsWith('/app-key')) {
					oauthWin.hide()
				}
			})
			...
			oauthWin.webContents.on('did-navigate', (evt, nextURL) => {
				const { hostname, pathname } = new URL(nextURL)
				if (hostname === 'andrews.com' && pathname.startsWith('/1/authorize')) {
